...
function normalizePhone(input: unknown): string {
  ...
  // Hapus semua angka 0 di depan
  s = s.replace(/^0+/, "");

  if (s.startsWith("+")) return s;

  // Jika sebelumnya angka 0 diganti jadi +62,
  // sekarang cek apakah awalnya 8 (setelah hapus 0)
...
export async function POST(req: Request) {
  try {
    ...
    if (password.length < 6) {
      return NextResponse.json(
        { ok: false, message: "Password minimal 6 karakter" },
        { status: 400 }
      );
    }
    ...
    if (!/^\d{6}$/.test(otp)) {
      return NextResponse.json(
        { ok: false, message: "OTP tidak valid" },
        { status: 400 }
      );
    }
    ...
    if (exists) {
      return NextResponse.json(
        { ok: false, message: "Nomor telepon sudah terdaftar" },
        { status: 409 }
      );
    }
    ...
    if (record.expiresAt.getTime() < Date.now()) {
      return NextResponse.json(
        { ok: false, message: "OTP sudah kadaluarsa" },
        { status: 400 }
      );
    }
    ...
    if (hasName(err) && err.name === "PrismaClientInitializationError") {
      return NextResponse.json(
        {
          ok: false,
          message: "Database bermasalah. Cek DATABASE_URL di Vercel.",
        },
        { status: 500 }
      );
    }
